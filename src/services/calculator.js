...
export default {
  calculateRateOfReturn: function (
    numberOfPeriods,
    payment,
    presentValue,
    futureValue = 0.0,
    ...
    var payment1 = this.calculateInternalRateOfReturn(
      rate1,
      numberOfPeriods,
      payment,
      presentValue,
    ...
    var payment2 = this.calculateInternalRateOfReturn(
      rate2,
      numberOfPeriods,
      payment,
      presentValue,
    ...
    do {
      ...
      if (payment2 == payment1) {
        ...
        payment1 = this.calculateInternalRateOfReturn(
          rate1,
          numberOfPeriods,
          payment,
          presentValue,
      ...
      var payment3 = this.calculateInternalRateOfReturn(
        rate3,
        numberOfPeriods,
        payment,
        presentValue,
  ...
  calculateInternalRateOfReturn: function (
    rate,
    numberOfPeriods,
    payment,
    presentValue,
    ...
    if (rate == 0.0) {
      ...
      result =
        presentValue * factor +
        (payment * adjustment * (factor - 1.0)) / rate +
        futureValue;
    }
    return result;
  },
  calculatePayment: function (
    ...
    if (rate == 0.0) {
      ...
      payment =
        ((-futureValue - presentValue * ratePlusOneToThePowerOfNper) /
          (dueFactor * (ratePlusOneToThePowerOfNper - 1.0))) *
        rate;
    }
    return payment;
  },
  calculateFutureValue: function (
    ...
    if (interestRate == 0.0) {
      ...
      futureValue =
        ...
        (payment / interestRate) * factor * (compoundFactor - 1.0);
    }
    return futureValue;
  },
  calculatePresentValue: function (
    interestRate,
    numberOfPeriods,
    payment,
    futureValue = 0.0,
    ...
    if (interestRate == 0.0) {
      ...
      presentValue =
        -(
          futureValue +
          payment * factor * ((compoundFactor - 1.0) / interestRate)
        ) / compoundFactor;
    }
    return presentValue;
  },
